...
public class CasinoWagersService
{
    ...
    public async Task<bool> InsertCasinoWagersAsync(List<Wager> wagers)
    {
        try
        {
            ...
            _logger.LogInformation("[#] Inserting into DB [#]");
            ...
            _logger.LogInformation("[#] Inserting into DB done![#] ");

            return true;
        }
        catch (DbUpdateException dbEx)
        {
            _logger.LogInformation($"Database update failed while inserting casino wagers with Exception: {dbEx}");
            return false;
        }
        catch (Exception ex)
        {
            _logger.LogError($"An unexpected error occurred while inserting casino wagers with Exception: {ex}");
            return false;
        }
    }
